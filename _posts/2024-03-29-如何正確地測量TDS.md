...
<div class="row mt-md-4 mt-3 mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-12">
...
2.   充分攪拌後取樣，總共取 10 個 5ml 的樣品，其中 5 個樣品為對照組（Control），以上一章節所介紹的標準做法進行取樣以及測量的前處理（放入樣品瓶後浸入純水冷卻），而其餘 5 個樣品則放在未加蓋的杯測碗中冷卻（以 No Lid 表示）。
...
以下是總共 10 個樣本的 TDS 測量結果，以及根據 TDS 數值、咖啡液重、粉重所計算出的萃取率。
...
<div class="row mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-12">
        <span class="image-span">未加蓋的實驗組（No Lid）與對照組（Control）各五個樣品的測量結果</span>
    </div>
</div>
...
<div class="row mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-12">
        <span class="image-span">TDS 及萃取率之平均值與標準差</span>
    </div>
</div>

下圖繪製了兩組數據的平均 TDS 及萃取率，其中 error bar 代表正負一個標準差的大小。

<div class="row mt-md-4 mt-3 mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-12">
        <img src="{{ site.github.url }}/assets/img/{{ page.imgfolder }}/fig1.webp" alt="" class="img-fluid">
        <span class="image-span">實驗組及對照組的平均 TDS 及萃取率</span>
    </div>
</div>
放在杯測碗中進行冷卻、沒有考慮蒸發量的實驗組有著較對照組高出 1.5% 的萃取率，經過 t-test 後也確認這個差距是具統計顯著性的，同時，我們也能觀察到對照組有著更好的測量穩定度（標準差較小）。

##### 討論

任何有在測量 TDS 的讀者應該都會同意，萃取率 1.5% 的差距是非常大的，而這僅僅是 30 分鐘冷卻所造成的差距，這顯示在測量時避免蒸發量的影響極為重要。而從實驗結果也可以看出，對照組所使用的標準取樣及前處理手法可以有效地增加測量時的穩定性。
...
下圖繪製了以三台 Difluid R2 以及一台 Atago PAL-COFFEE 分別測量 20 個樣本所得之數據，其中橫軸為樣本編號（從 1 到 20），縱軸為 TDS（單位為 %）。

<div class="row mt-md-4 mt-3 mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-12">
        <img src="{{ site.github.url }}/assets/img/{{ page.imgfolder }}/fig2.webp" alt="" class="img-fluid">
        <span class="image-span">以四台折射儀測量 20 個樣本</span>
    </div>
</div>
下表則呈現了各台機器的平均測量值及標準差。
...
<div class="row mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-12">
        <span class="image-span">四台折射儀各自的測量平均值與標準差</span>
    </div>
</div>
從折線圖及表格中能看出四台機器的測量值大致呈現一穩定的大小關係，即 Atago > Difluid #1 > Difluid #2 > Difluid #3，關於這點下一章會有更詳細的討論。

而我也將四台折射儀的測量值繪製成了如下的小提琴圖（violin plot），其中三條橫線由上到下分別為最大值、平均、最小值，而淺色的部分則為測量值的分布。


<div class="row mt-md-4 mt-3 mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-12">
        <img src="{{ site.github.url }}/assets/img/{{ page.imgfolder }}/fig3.webp" alt="" class="img-fluid">
        <span class="image-span">四台折射儀各自測量值的小提琴圖</span>
    </div>
</div>
透過小提琴圖我們可以發現三台 Difluid R2 的測量值範圍較窄，代表它們的精密度（Precision）較高，而 Atago 則略差一些，但四台折射儀測量值的標準差均為約 0.01%，表現已經大幅超越他們所標榜的精密度（Difluid 為 0.03％ 而 Atago 則為 0.15%）。
...
4.   加入約 90g 的 50°C 純水，總重為 108.02g（這個重量並不重要，冷卻後會再測量一次以排除蒸發的影響）。
...
下圖繪製了以三台 Difluid R2 測量 10 個樣本所得之數據，其中橫軸為樣本編號（從 1 到 10），縱軸為 refractive index，理論值（Ground Truth）以紅線標示。
...
<div class="row mt-md-4 mt-3 mb-md-6 mb-4 justify-content-center text-center">
    ...
    <div class="col-md-6 mt-md-0 mt-3">
        ...
        <div class="col-md-12">
        <span class="image-span">圖六、圖七：以三台 Difluid R2 測量 10 個糖溶液樣本<br>圖六與圖七分別為第一、二次實驗</span>
    </div>
</div>


下表則呈現了各台機器的平均測量值及標準差，以及與理論值相差的比例。<sup class="footnote-sup">[B]</sup>
...
| 與理論值相差 | -1.94%            | -2.08%            | -2.37%            |

| 實驗 3.2     | Difluid #1        | Difluid #2        | Difluid #3        |
| ------------ | ----------------- | ----------------- | ----------------- |
| 測量值       | 1.34749 ± 0.00001 | 1.34747 ± 0.00002 | 1.34741 ± 0.00001 |
| 與理論值相差 | -1.39%            | -1.51%            | -1.93%            |

<div class="row mb-md-4 mb-3 justify-content-center text-center">
    <div class="col-md-12">
        <span class="image-span">表四、表五：在兩次實驗中，三台 Difluid R2 所測量 refractive index 的平均值及標準差</span>
    </div>
</div>


可以發現三台 R2 的測量值均較理論值低，而大小關係為 #1 > #2 > #3，大致與實驗二之趨勢相同，這代表 R2 或許有所謂的「體質」之分，即某些機器會固定測出較低的值，某些則相反。而這也代表在實驗二中測量值較高的 Atago 或許更準確，但這還需要進一步的驗證。
...
<div class="row mt-md-4 mt-3 mb-md-6 mb-4 justify-content-center text-center">
    <div class="col-md-6">
        ...
        <span class="image-span">三台 Difluid 測量值的小提琴圖</span>
    </div>
    <div class="col-md-6 mt-md-0 mt-3">
...
可以看到三台 R2 的測量值浮動的範圍非常小（除了 #2 稍大一些），代表精密度（Precision）高，且三台數值雖有大小差異但卻也相差不多。而測量值偏低的結果也不排除是因為溶液製備上的問題，例如有些許砂糖並未完全溶解，或是砂糖的純度不夠高。
