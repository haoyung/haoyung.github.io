...
WDT involves using a needle-like tool to stir the coffee grounds, breaking up clumps, and ensuring even distribution. While it's commonly used for preparing espresso pucks, it has also gained popularity in pour-over brewing to level the initial coffee bed and aid during the bloom phase.

As WDT gained traction, there was natural curiosity to adapt it further. Wet WDT, as the name suggests, is a variation where the WDT tool is used during the pour-over process, typically during the bloom. However, it can be applied later in the brewing process as well (with caution to avoid clogging the filter!).
...
They observed that Wet WDT increased the average total brew time (TBT) from 266 seconds to 334 seconds. Remarkably, the average extraction yield increased from 21.66% to 23.33%—an impressive 1.67% boost.

This raises the question: **how much of this increase in extraction yield is actually due to the extended TBT?** Agitation tends to increase both TBT and yield. This is because agitation promotes fine migration, leading to filter clogging and a slower flow rate, which in turn increases TBT and yield.
...
From these conclusions, Wet WDT appears almost magical. It not only provides gentler agitation that's less likely to cause clogging but also significantly boosts extraction yield. Inspired by the article, like many coffee enthusiasts globally, I felt compelled to try stirring a few times with a WDT tool during the bloom.

But are these conclusions truly remarkable? Is Wet WDT genuinely that miraculous? Subsequent experiences led me to question this, prompting the motivation for this article. I aim to explore: 

<p class="mb-md-5 mb-4 mt-md-5 mt-4 quote">How much of the extraction yield increase from Wet WDT is actually related to the extended TBT? And how much is due to its 'magical' effects, such as increased extraction uniformity and improved mixing of coffee grounds and water?</p>
...
Initially, I was skeptical because I perceived Wet WDT as a very "gentle" agitation method, seemingly unsuitable for triggering fine migration and clogging.

<p class="mb-md-5 mb-4 mt-md-5 mt-4 quote">However, after trying it myself, I found I was completely wrong.</p>

With an extremely coarse grind, where the total brew time wouldn't exceed 2 minutes, using Wet WDT during the bloom easily clogged the filter, pushing TBT close to 10 minutes. This means that Wet WDT, contrary to my initial perception of being "gentle," actually involves intense agitation. Therefore, while it increases extraction yield, it also accelerates fine migration—a side effect (though in Rohan's parameters, this isn't a side effect but an intended outcome). In this regard, it's no different from other agitation-increasing methods.
...
Due to the complexity of this experiment, I conducted a simple pre-experiment to ensure the chosen parameters were stable enough. The main differences between the pre-experiment and the main experiment are the grouping method and the number of brews conducted.
...
10.  At 1:00, pour water to 230g at about 10 ml/s, then perform a swirl.
...
<div class="row mb-md-5 mb-4 justify-content-center text-center">
    <div class="col-md-12">
        <span class="image-span">Table 1: Total brew time and extraction yield for the three groups in the pre-experiment</span>
    </div>
</div>

From Table 1, we can observe:

1. Both WWDT and CTL-fast had the valve fully open during brewing, but WWDT had a significantly longer total brew time, confirming the earlier statement that WWDT accelerates fines migration.
2. WWDT and CTL-slow, which had similar total brew times, had similar extraction yields.
...
I will look for more evidence to support or refute these hypotheses in the main experiment.
