***Here is the revised version of the coffee blogpost:
...
However, we often need objective data, not only for communication and exchange among enthusiasts but also to help us adjust parameters and improve a subpar cup of coffee.
...
Since the extraction yield is calculated from TDS, measuring TDS has become the most crucial step in obtaining *objective* data about coffee. However, accurately measuring TDS is extremely difficult. Even slight flaws in the process can lead to significant errors in the results. On Instagram or YouTube, it's common to see videos of measurements being taken in absolutely incorrect ways, causing these *objective* data measurements to no longer be *objective*, losing their original meaning.
...
Generally, we cool the device, the distilled water for zeroing, and the coffee sample to room temperature before measuring. However, this is much more difficult than it sounds because even if we place all three in the same indoor location for a while, it is still challenging to ensure their temperatures are exactly the same. In the next section on the measurement process, I will detail how to cool the samples.
