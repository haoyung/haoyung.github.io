...
3.   加入約 10g 的白砂糖，最後重量為 224.68g，代表加入了 224.68 - 214.67 = 10.01 (g) 的白砂糖。

4.   加入約 90g 的純水，最後重量為 314.90g（這個重量並不重要，冷卻後會再測量一次以排除蒸發量）。
...
7.   可以利用下表（同樣謝謝 @quantitativecafe 的[數據](https://github.com/quantitativecafe/blog/blob/main/refractive-index/refractive-index.xlsx)）以及[內插法](https://zh.wikipedia.org/zh-tw/%E6%8F%92%E5%80%BC)算出理論上的 refractive index（也就是 R2 測量到的原始數值，會顯示在螢幕的左上角）。
...
8.   根據上表，可以用內插法算出理論上的 refractive index 為：

     $$ 1.3478 + (1.3509-1.3478)*\displaystyle\frac{10.027-10}{12-10}\approx1.34784 $$

9.   而我們的兩台的 R2 測量值則分別為 1.34769 與 1.34762，確實都偏低，但與理論值並無太大的差距。

只可惜 R2 目前沒有校正的功能，因此這個作法也只能檢測，而沒有辦法實際改善機器的精準度。不過知道自己的機器可以測量出與理論值相近的值應該還是一件安心的事。
