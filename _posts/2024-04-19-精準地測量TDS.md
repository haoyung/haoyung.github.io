...
### 實驗四：使用標準液檢測 Difluid R2 Extract 的精準度（更新於 06/19/2024）

在這篇文章釋出後，Difluid 台灣的官方人員主動聯繫了我（可以追蹤他們的 [IG](https://www.instagram.com/difluid.tw)！），並表示可以提供他們官方所使用的標準液讓我進行測試，非常感謝他們！

#### 實驗步驟

我總共收到了兩瓶標準液，濃度分別為 1.21% 及 12.65%。因為他們都裝在密封的眼藥水瓶內，因此我沒有進行取樣，而僅簡單地利用三台 Difluid 對兩瓶標準液分別進行 10 次測量，並記錄數據。為了方便區分，我會將測量淡標準液（1.21%）的實驗稱為實驗 4.1，而測量濃標準液（12.65%）的實驗稱為實驗 4.2。

#### 實驗結果

圖十一與圖十二繪製了在實驗 4.1 與 4.2 中以三台 Difluid 測量兩瓶標準液各 10 次所得之數據，其中橫軸為測量編號（從 1 到 10），縱軸為 TDS，瓶身上所標示的 TDS 理論值（ground truth）以紅色虛線標示。
...
<div class="row mt-md-5 mt-4 mb-md-5 mb-4 justify-content-center text-center">
    ...
    <span class="image-span">圖十一、圖十二：實驗 4.1 與 4.2 中的測量值折線圖及標準液瓶身所標示的 TDS 理論值</span>
</div>
...
可以發現三台 Difluid 的測量值均較理論值低，而大小關係為 #2 > #1 > #3，與實驗一、三之趨勢稍有不同，我同時也將測量數據繪製成了如下的小提琴圖：
...
<div class="row mt-md-5 mt-4 mb-md-5 mb-4 justify-content-center text-center">
    ...
    <span class="image-span">圖十三、圖十四：實驗 4.1 與 4.2 中測量值的小提琴圖及及標準液瓶身所標示的 TDS 理論值</span>
</div>

我也將圖十三、圖十四與圖十中的四張子圖一同繪製成了圖十五，對照後可以發現三台 Difluid 的測量結果依然有著類似的趨勢，進一步增加了我對我的猜測『每台 Difluid 有著「體質」之分』的信心。
